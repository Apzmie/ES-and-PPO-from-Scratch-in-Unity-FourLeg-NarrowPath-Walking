...
One limitation was that during further training with PPO, the maximum average reward achieved good performance, but it failed to improve beyond that performance despite having room for further improvement. This may be caused by two factors: zero gradient and reduced exploration. Parameter updates do not occur when the policy is already moving strongly in the correct direction, and exploration is gradually reduced once the agent discovers actions that obtain high rewards, preventing further improvement.
